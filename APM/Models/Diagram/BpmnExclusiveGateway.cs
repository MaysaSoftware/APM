...
namespace APM.Models.Diagram
{
    public class BpmnExclusiveGateway
    {
        ...
        public List<BpmnOutgoing> bpmnOutgoingList { get; set; }
        public BpmnExclusiveGateway()
        {

        }
        public BpmnExclusiveGateway(string XmlStr)
        {
            ID = String.Empty;
            Name = String.Empty;
            bpmnIncomingsList = new List<BpmnIncoming>();
            bpmnOutgoingList = new List<BpmnOutgoing>();
            ...
            if (XmlStr.IndexOf("<bpmn:outgoing") > 0)
            {
                ...
                foreach (string StrItem in BpmnObjectArr)
                    if (StrItem.IndexOf("bpmn:outgoing") > -1)
                        bpmnOutgoingList.Add(new BpmnOutgoing(StrItem));
            }
        }
    }
}