            ...
            rules: {
                required: function (input) {
                    var noNameCheckbox = !input.attr("name") && !input.is(":checked"),
                        ...
                        value = input.val();
                    var ishidden = $(input).closest(".k-widget.IElement").is(":hidden");

                    return (ishidden || !(hasAttribute(input, "required") && (!value || value === "" || value.length === 0 || checkbox || radio)));
                },
                pattern: function (input) {
                    if (input.filter("[data-role=textbox]").filter("[pattern]").length && input.val() !== "") {
                        var ishidden = $(input).closest(".k-widget.IElement").is(":hidden");
                        return (ishidden || patternMatcher(input.val(), input.attr("pattern")));
                    }
                    return true;
                },
                min: function(input) {
                ...
                email: function(input) {
                    var ishidden = $(input).closest(".k-widget.IElement").is(":hidden");
                    return (ishidden || matcher(input, "[type=email],[" + kendo.attr("type") + "=email]", emailRegExp));
                },
                url: function(input) {
                    var ishidden = $(input).closest(".k-widget.IElement").is(":hidden");
                    return (ishidden || matcher(input, "[type=url],[" + kendo.attr("type") + "=url]", urlRegExp));
                },
                date: function (input) {
                    if (input.hasClass("pwt-datepicker-input-element") && input.val() !== "") {
                        var ishidden = $(input).closest(".k-widget.IElement").is(":hidden");
                        return (ishidden || (input.val().match(/(13|14)(\d{2})\/((0[1-6]\/((0[1-9])|([1-2][0-9])|(3[0-1])))|((1[0-2]|(0[7-9]))\/((0[1-9])|([1-2][0-9])|30)))/) != null));
                    }
                    return true;
                },
                dropdownlistrequired: function (input) {
                    if (input.is("[data-role=dropdownlist]") && input.is("[required]")) {
                        var ishidden = $(input).closest(".k-widget.IElement").is(":hidden");
                        return (ishidden || (input.val() !== "0"));
                    }
                    return true;
                },
                exclusive: function (input) {
                    if ((input.is("[data-role=textbox]") || input.is("[data-role=maskedtextbox]")) && input.attr("data-exclusive")) {
                        var CoreObjectID = input.data("coreobjectid");
                        var value = "";
                        var result = null;

                        switch (true) {
                            case input.is("[data-role=textbox]"):
                                value = input.data("kendoTextBox").value();
                                break;

                            case input.is("[data-role=maskedtextbox]"):
                                value = input.data("kendoMaskedTextBox").value();
                                break;
                        }

                        if (value != "") {
                            var recordID = $("#شناسه").data("kendoNumericTextBox").value() || 0;
                            var ishidden = $(input).closest(".k-widget.IElement").is(":hidden");
                            if (ishidden) { return true; }

                            result = $.ajax({
                                url: "/DataManager/ExclusiveValidate",
                                type: "POST",
                                async: false,
                                cache: false,
                                timeout: 50000,
                                 data: { _CoreObjectID: CoreObjectID, _RecordID: recordID, _Value: value}
                            });
                ...
                custom: function (input) {
                    if ((input.is("[data-role=textbox]") || input.is("[data-role=dropdownlist]")) && input.attr("data-customvalidation")) {
                        var ishidden = $(input).closest(".k-widget.IElement").is(":hidden");
                        if (ishidden) { return true; }

                        var CoreObjectID = input.data("coreobjectid");
                        var customvalidation = new Function("input", "return customvalidation" + CoreObjectID + "(input);")
